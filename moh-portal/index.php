<?php
    ...
    if (isset($_SESSION['SESSION_EMAIL'])) {
        header("Location: ./moh-citizens/citizens.php");
        die();
    }
    ...
    if (isset($_GET['verification'])) {
        if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM moh_admin WHERE code='{$_GET['verification']}'")) > 0) {
            $query = mysqli_query($conn, "UPDATE moh_admin SET code='' WHERE code='{$_GET['verification']}'");
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "SELECT * FROM moh_admin WHERE mohadmin_email='{$email}' AND password='{$password}'";
        ...
        if (mysqli_num_rows($result) === 1) {
            ...
            if (empty($row['code'])) {
                ...
                header("Location: ./moh-inquirers/inquirers.php");
            } else {
...
<head>
    ...
    <title> MoH Login | CoVaxPassTT </title>

    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta charset="UTF-8" />
    <meta name="keywords" content="MoH Login Form" />
